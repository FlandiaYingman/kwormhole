...
import top.anagke.kwormhole.model.Metadata.Companion.utcTimeMillis
...
class KwormholeAppTests {
    ...
    private lateinit var testMetadataList: List<Metadata>

    private lateinit var testData: List<Pair<Metadata, ByteArray>>
    ...
    fun doPut() {
        ...
        testMetadataList = List(TEST_COUNT) { index ->
            Metadata(Random.nextPath(TEST_DEPTH), testContentList[index].hash(), Random.nextLong(utcTimeMillis))
        }
    ...
    @Test
    fun testList() {
        val list = restTemplate.exchange<List<Metadata>>(path("list"), GET).body
        assertThat(list)
    ...
    fun testGet() {
        testData.forEach { (metadata, content) ->
            val actualMetadata = restTemplate.getForObject<Metadata>(path(metadata.path, "method" to "metadata"))
